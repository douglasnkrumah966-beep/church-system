...
.cat-att-hero h1 { font-size: 20px; margin: 0 0 4px; font-weight: 800; }
.cat-att-hero p  { font-size: 13px; opacity: 0.9; margin: 0; }
...
.cat-att-session:hover { background: #fff; border-color: #cbd5e0; }
.cat-att-session.active { background: #fffaf0; border-color: #f6ad55; }
...
.cat-att-info { flex: 1; min-width: 0; }
.cat-att-name { font-size: 14px; font-weight: 700; color: #2d3748; }
.cat-att-meta { font-size: 11px; color: #718096; margin-top: 2px; }
.cat-att-stats {
  display: flex;
  gap: 4px;
  margin-top: 4px;
...
.cat-att-stat.present { background: #c6f6d5; color: #22543d; }
.cat-att-stat.excused { background: #feebc8; color: #7b341e; }
.cat-att-stat.absent  { background: #fed7d7; color: #742a2a; }
...
<!-- ============================================================
     HERO
     ============================================================ -->
<div class="cat-att-hero">
  <h1>📋 Attendance</h1>
  <p>Track who attends each catechism session. Perfect for Sunday school and sacrament preparation classes.</p>
</div>

<!-- ============================================================
     CREATE NEW SESSION
     ============================================================ -->
<div class="cat-att-section">
  <h3>➕ Create New Session</h3>
  <p class="subtitle">
    Give it a title, pick a date, and mark attendance for that class.
  </p>
    ...
    <div class="grid grid-2">
      ...
      <div>
        ...
        <select name="category">
          ...
          <option>First Communion Class</option>
          <option>Confirmation Class</option>
...
<?php if (is_array($current)): ?>
  ...
  <div class="cat-att-section" style="border-left:4px solid #dd6b20;">
    <h3>
      ✅ Mark Attendance
    ...
    <form method="post">
      ...
      <div style="position:sticky;bottom:10px;margin-top:20px;text-align:right;">
        <button class="btn-primary" name="save_attendance" value="1"
                style="max-width:280px;background:#dd6b20;">
          💾 Save Attendance
        </button>
      </div>
    </form>
...
<div class="cat-att-section">
  ...
  ?>
    <div class="cat-att-session <?= $isActive ? 'active' : '' ?>">
      <div class="cat-att-icon">
        <?= e(strtoupper(substr($s['category'] ?? '?', 0, 1))) ?>
      </div>
      <div class="cat-att-info">
        <div class="cat-att-name"><?= e($s['title'] ?? 'Untitled') ?></div>
        <div class="cat-att-meta">
          <?= !empty($s['session_date'])
              ? 'date'('jS M Y', 'strtotime'($s['session_date']))
              : '—' ?>
          · <?= e($s['category'] ?? '') ?>
        </div>
        <div class="cat-att-stats">
          <span class="cat-att-stat present">✓ <?= $stats['present'] ?> present</span>
          <span class="cat-att-stat excused">🕊 <?= $stats['excused'] ?> excused</span>
          <span class="cat-att-stat absent">✕ <?= $stats['absent'] ?> absent</span>
        </div>
      </div>
      <a class="btn-sm" style="padding:10px 16px;font-size:13px;background:#dd6b20;color:#fff;"
         href="?id=<?= $sid ?>">
...
<!-- ============================================================
     INFO NOTE
     ============================================================ -->
<div class="cat-att-section" style="background:#fffaf0;border-left:4px solid #dd6b20;">
  <h3 style="color:#7b341e;">💡 Catechism Attendance Tips</h3>
  <ul style="font-size:13px;color:#7b341e;line-height:1.9;padding-left:20px;margin-top:8px;">
    <li><strong>Create a session</strong> for each class — e.g. "Sunday Catechism — Class 3"</li>
    <li><strong>Mark Present / Excused</strong> for each student before class ends</li>
    <li><strong>Excused</strong> = absent with reason (sick, family emergency, etc.)</li>
    <li><strong>Absent</strong> = did not attend without notice</li>
    <li><strong>Track patterns</strong> — repeated absences may need follow-up with parents</li>
    <li>Use <strong>Attendance Report</strong> in the nav to see overall attendance rates</li>
  </ul>
</div>
